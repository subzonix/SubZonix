...
export function generateInvoicePDF(sale: any, companyInfo?: { name?: string, slogan?: string, logo?: string, contact?: string, account?: string, iban?: string, bankName?: string, accountHolder?: string, loginLink?: string }) {
    ...
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Phone: ${sale.client?.phone || "N/A"}`, 20, 89);
    ...
    const footerY = 297 - footerHeight; // 297 is A4 page height, position footer at very bottom
    ...
    // White text for visibility on indigo background
    ...
    if (companyInfo?.contact) {
        doc.setFont("helvetica", "normal");
        doc.setFontSize(8);
        doc.text(`📱 WhatsApp: ${companyInfo.contact}`, 20, footerY + 14);
    }
