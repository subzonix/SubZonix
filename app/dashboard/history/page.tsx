...
import { cleanPhone, exportToCSV, toHumanDate } from "@/lib/utils";
...
export default function HistoryPage() {
    ...
    const groupedClients = useMemo(() => {
        ...
        return Object.values(groups).filter(g =>
            ...
            g.client.phone.includes(search)
        );
    }, [sales, search]);
    ...
    const purchaseStats = useMemo(() => {
        return purchaseSales.reduce((acc, s) => ({
            count: acc.count + 1,
    ...
    const handleDelete = async (id: string) => {
        ...
        if (ok) {
            try {
                if (!user) return;
                await deleteDoc(doc(db, "users", user.uid, "salesHistory", id));
    ...
    const handleImportCSV = async () => {
        ...
        try {
            ...
            const salesRef = collection(db, "users", user.uid, "salesHistory");
            let successCount = 0;
    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-3xl overflow-hidden shadow-sm transition-all duration-300">
                <div
                    onClick={() => setShowDailyPurchasing(!showDailyPurchasing)}
                    className="p-6 cursor-pointer hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
                >
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h2 className="text-lg font-black text-[var(--foreground)] tracking-tight">Daily Selling Details</h2>
                    ...
                    {!showDailyPurchasing && (
                        <div className="flex items-center gap-4 px-4 py-2  rounded-2xl border border-[var(--border)] animate-in fade-in zoom-in duration-300">
                    ...
                    {showDailyPurchasing && (
                        <div className="flex flex-wrap items-center gap-3 bg-[var(--card)] p-2 rounded-2xl border border-[var(--border)] shadow-sm animate-in slide-in-from-right-4 duration-300" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center gap-2">
                                ...
                                <input
                                    ...
                                    className="bg-slate-100 dark:bg-slate-800 text-[11px] px-3 py-1.5 rounded-xl border border-transparent focus:border-emerald-500 transition-all outline-none text-[var(--foreground)]"
                                />
                            </div>
                            ...
                            <div className="flex items-center gap-5 px-2">
                                <div className="text-center">
                                    ...
                                    <div className="text-[11px] font-black text-[var(--foreground)]">{purchaseStats.count}</div>
                                </div>
                                <div className="text-center">
                ...
                {showDailyPurchasing && (
                    <div className="border-t border-[var(--border)] animate-in slide-in-from-top-4 duration-300">
                        <div className="overflow-x-auto max-h-[400px] custom-scrollbar">
                            ...
                            <div className="md:hidden space-y-4 p-4">
                                {loading ? (
                                    ...
                                    purchaseSales.map((sale, idx) => (
                                        <div key={sale.id || idx} className="bg-[var(--background)] p-4 rounded-xl border border-[var(--border)]">
                                            <div className="flex justify-between items-start mb-3">
                                                <div>
                                                    <div className="font-black text-[var(--foreground)]">{sale.client.name}</div>
                                                ...
                                                <div className="text-right">
                                                    <div className="font-bold text-slate-700 dark:text-slate-300 text-xs">{sale.vendor.name}</div>
                                                    <div className={clsx("text-[9px] font-black uppercase tracking-tight", sale.vendor.status === "Paid" ? "text-slate-400" : "text-rose-500")}>
                                                        {sale.vendor.status}
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="flex flex-wrap gap-1 mb-3">
                                                {sale.items.map((item, k) => (
                                    ...
                                    {loading ? (
                                        ...
                                        purchaseSales.map((sale, idx) => (
                                            <tr key={sale.id || idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/5 transition">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="font-bold text-slate-700 dark:text-slate-300">{sale.vendor.name}</div>
                                                    <div className={clsx("text-[9px] font-black uppercase tracking-tight", sale.vendor.status === "Paid" ? "text-slate-400" : "text-rose-500")}>
                                                        Status: {sale.vendor.status}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
            ...
            <div className="space-y-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-[var(--card)] p-4 rounded-2xl border border-[var(--border)] shadow-sm gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                        ...
                        <PlanFeatureGuard
                            feature="export"
                            fallback={
                                <button className="btn-save text-[10px] px-5 py-3 flex items-center gap-2 transition-all duration-200 ease-out active:scale-[0.97] w-full sm:w-auto justify-center opacity-50 cursor-not-allowed" title="Upgrade plan to export">
                        ...
                        >
                            <button
                                ...
                                className="btn-save text-[10px] px-5 py-3 flex items-center gap-2 transition-all duration-200 ease-out active:scale-[0.97] w-full sm:w-auto justify-center"
                            >
                                <FaFileCsv className="text-sm" />
                                Export Data
                            </button>
                        ...
                        <PlanFeatureGuard
                            feature="importData"
                            fallback={
                                <button className="btn-whatsapp text-[10px] px-5 py-3 flex items-center gap-2 transition-all duration-200 ease-out active:scale-[0.97] w-full sm:w-auto justify-center opacity-50 cursor-not-allowed" title="Upgrade plan to import">
                        ...
                        >
                            <button
                                ...
                                className="btn-whatsapp text-[10px] px-5 py-3 flex items-center gap-2 transition-all duration-200 ease-out active:scale-[0.97] w-full sm:w-auto justify-center"
                            >
                                <FaFileImport className="text-sm" />
                                Import CSV
                            </button>
                ...
                <div className="space-y-3">
                    {loading ? (
                        ...
                        groupedClients.map((group) => (
                            <div key={group.client.phone} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm transition-all duration-300 group">
                                <div
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="font-black text-sm text-[var(--foreground)] tracking-tight">{group.client.name}</div>
                                    ...
                                    <div className="flex items-center gap-6">
                                        <div className="text-right">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <div className="text-xs font-black text-[var(--foreground)]">{group.sales.length} Orders</div>
                                ...
                                {expandedClient === group.client.phone && (
                                    <div className="border-t border-[var(--border)] p-4 bg-slate-50/30 dark:bg-slate-800/10 animate-in slide-in-from-top-2 duration-300">
                                        <div className="space-y-3">
                                            {group.sales.map((s) => (
                                                <div key={s.id} className="bg-[var(--card)] p-4 rounded-2xl border border-[var(--border)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm hover:shadow-md transition-all">
                                                    <div className="flex-1 w-full overflow-hidden">
                                                        ...
                                                        <div className="overflow-x-auto max-w-[75vw] sm:max-w-full">
                                                            <table className="w-full text-[10px] text-left">
                                                                <thead className="text-slate-400 font-black uppercase tracking-widest border-b border-[var(--border)]">
                                                                    <tr>
                                                                ...
                                                                <tbody className="divide-y divide-[var(--border)] text-slate-600 dark:text-slate-400">
                                                                    {s.items.map((i, k) => (
                                                                        <tr key={k} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                                                                            <td className="py-2.5 pr-4 font-black text-[var(--foreground)]">{i.name}</td>
                                                    ...
                                                    <div className="flex items-center gap-2 w-full sm:w-auto justify-end border-t sm:border-t-0 pt-3 sm:pt-0">
                                                        <button onClick={() => setSelectedSale(s)} className="flex items-center gap-2 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-indigo-600 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl hover:bg-indigo-600 hover:text-white transition-all border border-indigo-100 dark:border-indigo-800/50 cursor-pointer" title="View Details">
                                                            <FaEye /> View
                                                        </button>
                                                        <PlanFeatureGuard feature="pdf" fallback={<button className="btn-pdf p-2.5 rounded-xl cursor-not-allowed opacity-50"><FaFilePdf /></button>}>
                                                            <button onClick={() => handleDownloadPDF(s, companyInfo)} className="btn-pdf p-2.5 rounded-xl cursor-pointer" title="PDF"><FaFilePdf /></button>
                                                        </PlanFeatureGuard>
                                                        <PlanFeatureGuard feature="whatsappAlerts" fallback={<button className="btn-whatsapp p-2.5 rounded-xl cursor-not-allowed opacity-50"><FaWhatsapp /></button>}>
                                                            <button onClick={() => window.open(`https://wa.me/${cleanPhone(s.client?.phone || "")}`, '_blank')} className="btn-whatsapp p-2.5 rounded-xl cursor-pointer" title="WhatsApp"><FaWhatsapp /></button>
                                                        </PlanFeatureGuard>
                                                        <button onClick={() => router.push(`/new-sale?id=${s.id}`)} className="btn-edit" title="Edit"><FaPen className="text-sm" /></button>
                                                        <button onClick={() => handleDelete(s.id!)} className="btn-delete" title="Delete"><FaTrash /></button>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                        ...
                        groupedTools.map((group) => (
                            <div key={group.tool} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm transition-all duration-300 group">
                                <div
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="font-black text-sm text-[var(--foreground)] tracking-tight uppercase">{group.tool}</div>
                                    ...
                                    <div className="border-t border-[var(--border)] p-4 bg-slate-50/30 dark:bg-slate-800/10 animate-in slide-in-from-top-2 duration-300">
                                        <div className="space-y-4">
                                            {group.sales.map((s, idx) => (
                                                <div key={`${s.id}-${idx}`} className="bg-[var(--card)] p-4 rounded-2xl border border-[var(--border)] shadow-sm hover:shadow-md transition-all">
                                                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                                                    ...
                                                                    <tbody className="divide-y divide-[var(--border)] text-slate-600 dark:text-slate-400">
                                                                        <tr className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                                                                            <td className="py-2.5 pr-4">
                                                                                <div className="font-black text-[var(--foreground)]">{s.client.name}</div>
                                                                            ...
                                                                            <td className="py-2.5 text-right font-mono text-rose-500">Rs. {s.items[s.itemIndex].cost}</td>
                                                                            <td className="py-2.5 text-right font-black text-emerald-600 font-mono">Rs. {s.items[s.itemIndex].sell}</td>
                                                                        </tr>
                                                                    </tbody>
                                                                </table>
                                                            </div>
                                                        ...
                                                        <div className="flex items-center gap-2 w-full sm:w-auto justify-end border-t sm:border-t-0 pt-3 sm:pt-0">
                                                            ...
                                                            <PlanFeatureGuard feature="pdf" fallback={<button className="btn-pdf cursor-not-allowed opacity-50"><FaFilePdf /></button>}>
                                                                <button onClick={() => handleDownloadPDF(s, companyInfo)} className="btn-pdf" title="PDF"><FaFilePdf /></button>
                                                            </PlanFeatureGuard>
                                                            <PlanFeatureGuard feature="whatsappAlerts" fallback={<button className="btn-whatsapp cursor-not-allowed opacity-50"><FaWhatsapp /></button>}>
                                                                <button onClick={() => window.open(`https://wa.me/${cleanPhone(s.client.phone)}`, '_blank')} className="btn-whatsapp" title="WhatsApp"><FaWhatsapp /></button>
                                                            </PlanFeatureGuard>
                                                            <button onClick={() => router.push(`/dashboard/new-sale?id=${s.id}`)} className="btn-edit" title="Edit"><FaPen className="text-sm" /></button>
                                                            <button onClick={() => handleDelete(s.id!)} className="btn-delete" title="Delete"><FaTrash /></button>
                                                        </div>
                                                    </div>
                                                </div>
            ...
            {showExportModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-[var(--card)] border border-[var(--border)] rounded-3xl shadow-2xl w-full max-w-[800px] min-h-[550px] overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            {showImportModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-[var(--card)] border border-[var(--border)] rounded-3xl p-6 w-[600px] shadow-2xl">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-3 py-2 border border-[var(--border)] rounded-xl bg-[var(--background)] text-sm"
                                />
                                {importFileReal && (
